 ...
 */
async function main(repoOwner, repoName, csvFile, githubToken, config = {}) {

  const { batchSize = 10, batchDelayMs = 2000 } = config;
  ...
  for (let i = 0; i < issues.length; i += batchSize) {
    ...
    console.log(`Processing batch ${Math.floor(i / batchSize) + 1} (${batch.length} issues)`);

    const batchResults = await Promise.allSettled(
      batch.map((issue) => {
        if (issue.Title && issue.Title.trim()) {
          return createIssue(issue, repoOwner, repoName, githubToken);
        }
    ...
    batchResults.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        failed.push({ issue, error: result.reason });
      }
    });
